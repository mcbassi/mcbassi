...
$langParam = function_exists('current_lang') ? current_lang() : 'pt';
if (!preg_match('/(?:^|[?&])lang=/', $embedUrl)) {
    $embedUrl .= (str_contains($embedUrl, '?') ? '&' : '?') . 'lang=' . rawurlencode($langParam);
}
?>
<style>
.ppt-embed-shell {
...
<div class="ppt-embed-shell">
    <section class="ppt-embed-card">
        <header class="ppt-embed-card__head">
            <h2 class="ppt-embed-card__title"><?= h(t('ppt.generator')) ?></h2>
            <p class="ppt-embed-card__subtitle">
                <?= h(t('ppt.embed_subtitle_prefix')) ?> <code>ppt_dynamic_builder</code> <?= h(t('ppt.embed_subtitle_suffix')) ?>
            </p>
        </header>

        <div class="ppt-embed-frame-wrap">
            <div id="ppt-embed-loading" class="ppt-embed-loading"><?= h(t('ppt.loading_presentations_module')) ?></div>

            <iframe
                id="ppt-embed-frame"
                class="ppt-embed-frame"
                src="<?= h($embedUrl) ?>"
                title="<?= h(t('ppt.dynamic_builder_title')) ?>"
                loading="eager"
