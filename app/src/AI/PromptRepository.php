...
final class PromptRepository
{
    ...
    public function save(array $payload): int
    {
        ...
        if ($id > 0) {
            $stmt = $this->pdo()->prepare('
                UPDATE prompts
                SET assistente = :assistente, funcao = :funcao, descricao = :descricao, prompt = :prompt, updated_at = NOW()
                WHERE id = :id
            ');
            $stmt->execute([
        ...
        $stmt = $this->pdo()->prepare('
            INSERT INTO prompts (assistente, funcao, descricao, prompt, updated_at)
            VALUES (:assistente, :funcao, :descricao, :prompt, NOW())
        ');
        $stmt->execute([
            ':assistente' => $assistente,
            ':funcao' => $funcao,
    ...
    public function formFields(): array
    {
        ...
        $labelSelect = $this->columnExists('form_fields', 'label')
    ...
    private function assertPromptsTable(): void
    {
        ...
        if (!$this->columnExists('prompts', 'updated_at')) {
            $this->pdo()->exec('ALTER TABLE `prompts` ADD COLUMN `updated_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP AFTER `prompt`');
            $this->pdo()->exec('UPDATE `prompts` SET `updated_at` = COALESCE(`updated_at`, NOW())');
        }
    }
